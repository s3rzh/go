...
// При передачи массива в функции - они копируются (те если это долшой массив - то будет создан такой же массив в памяти). Чтобы избежать, передаём указатель на массив &arr или делаем срез из массива arr[:]
...
// go run -gcflags='-m' ./main.go | grep escape
