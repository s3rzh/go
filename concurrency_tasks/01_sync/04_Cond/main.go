...
// Wait() : этот метод заставляет вызывающую горутину ждать, пока другая горутина не сообщит об условной переменной. Когда горутина вызывает Wait() , она снимает связанную блокировку и приостанавливает выполнение до тех пор, пока другая горутина не вызовет Signal() или Broadcast() для той же переменной sync.Cond .

// Signal() : этот метод пробуждает одну горутину, ожидающую условную переменную. Если ожидают несколько горутин, пробуждается только одна из них. Выбор того, какая горутина будет пробуждена, произволен и не гарантирован.
